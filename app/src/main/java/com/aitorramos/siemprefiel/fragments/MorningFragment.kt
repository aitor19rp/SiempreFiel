...
import kotlinx.android.synthetic.main.dialog_turn.*
...
class MorningFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        _view = inflater.inflate(R.layout.fragment_morning, container, false)
    ...
    private fun getTurns(){
        ...
        turnDB
            ...
            .addOnFailureListener{exception ->
                ...
                Log.e("Prueba", exception.toString())
            }
    }
    ...
    private fun setAddTurnDialog(){
        ...
        val builder = AlertDialog.Builder(context).setView(dialogView).setTitle("Añadir al turno de mañana")
        builder
    ...
    private fun setAddRequestDialog(){
        ...
        val dialogView = LayoutInflater.from(context).inflate(R.layout.dialog_turn, null)
        val builder = AlertDialog.Builder(context).setView(dialogView).setTitle("Solicitar turno ${daysOfWeek[day.toInt() -1]} mañana")
        builder
            .setPositiveButton("Aceptar", null)
            .setNegativeButton("Cancelar"){_, _ ->
    ...
    private fun updateWeekAdd(range: String){
        weekDB
            ...
            .addOnSuccessListener { documents ->
                for(document in documents){
                    val total:Long = document.get("totalM") as Long +1
                    weekDB.document(document.id).update("totalM", total)
                    if(range == "1") {weekDB.document(document.id).update("veteranM", true)}
                }
            }
    }
            ...
            .addOnSuccessListener { documents ->
                for (document in documents) {
                    ...
                    if (range == "1" && totalVeteran == 1) {
                        weekDB.document(document.id).update("veteranM", false)
                            .addOnSuccessListener {
                                getTurns()
                            }
                    }
